resource "fortisase_private_access_network_configuration" "bgp" {
  count = var.BGP != null && !var.BGP.bypass ? 1 : 0
...
resource "fortisase_private_access_service_connection" "sc_1" {
  count = var.SC != null && length(var.SC) > 0 ? 1 : 0
...
resource "fortisase_private_access_service_connection_region_cost" "sc_1_cost" {
  count = var.SC != null && length(var.SC) > 0 ? 1 : 0

  entries = tomap({
    for region in sort(keys(fortisase_private_access_service_connection.sc_1[0].config.region_cost)) :
    region => tomap({
      (fortisase_private_access_service_connection.sc_1[0].id) : var.SC[0].priority
    })
  })

  depends_on = [fortisase_private_access_service_connection.sc_1]
}

resource "fortisase_private_access_service_connection" "sc_2" {
  count = var.SC != null && length(var.SC) > 1 ? 1 : 0

  depends_on = [fortisase_private_access_service_connection_region_cost.sc_1_cost]
  ...
  backup_links = var.SC[1].backup_link ? [
    {
      create = [
        {
...
resource "fortisase_private_access_service_connection_region_cost" "sc_2_cost" {
  count = var.SC != null && length(var.SC) > 1 ? 1 : 0

  entries = tomap({
    for region in sort(keys(fortisase_private_access_service_connection.sc_2[0].config.region_cost)) :
    region => tomap({
      (fortisase_private_access_service_connection.sc_2[0].id) : var.SC[1].priority
    })
  })

  depends_on = [fortisase_private_access_service_connection.sc_2]
}
